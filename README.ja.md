...
- 画面上部の「経路」で、TypeSafe API と Vercel AI Gateway を切り替えられる
...
| `src/server/` | Bun のサーバー。キーの解決と、TypeSafe API・Gateway・小型 LLM の 3 つの上流 |
| `src/web/runner/` | 学習者のコードを Web Worker で実行する仕組み |
| `src/web/lessons/` | レッスンの本文と課題の判定。初期コードは `code/ja/*.js` と `code/en/*.js` |
| `src/web/app/` | React の画面。画面の文言は `i18n.tsx` にまとめている |

レッスンを足すときは、`src/web/lessons/code/ja/` と `code/en/` に初期コードを置き、各部のファイルに両方の言語で本文を書きます。
テストが次の 2 点を確かめます。

- 全レッスンの初期コードが、偽の API で最後まで動く（構文やプロパティ名の誤りを検出する）
- 日本語版と英語版の初期コードが、API に同じ内容を送る

`TYPESAFE_BASE_URL` を設定すると、TypeSafe API の接続先を差し替えられます（公式 SDK と同じ変数名）。
